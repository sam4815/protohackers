...
fn handle_connection(stream: TcpStream, queue_manager: Arc<Mutex<QueueManager>>) -> Result<()> {
    ...
    for line in reader.lines() {
        match serde_json::from_str(&line?) {
            Ok(Request::Put { job, queue, pri }) => {
                let id = queue_manager.try_lock().unwrap().add_job(queue, job, pri);
                writeln!(
                    writer,
                    "{}",
            ...
            Ok(Request::Delete { id }) => {
                ...
                let status = queue_manager.try_lock().unwrap().delete_job(id);
                writeln!(
                    writer,
                    "{}", 
            ...
            Ok(Request::Abort { id }) => {
                if active_jobs.contains_key(&id) {
                    ...
                    let status = queue_manager.try_lock().unwrap().abort_job(job);
    ...
    for job in active_jobs.into_values() {
        queue_manager.try_lock().unwrap().abort_job(job);
    }
